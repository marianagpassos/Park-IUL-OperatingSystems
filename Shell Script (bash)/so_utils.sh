...
so_debug () {
    ...
    >&2 echo -ne "${COLOUR_BACK_FAINT_GRAY}@DEBUG {Ln:$(caller)}${COLOUR_GRAY}" && for arg in "$@"; do >&2 echo -ne " [$arg]"; done; >&2 echo -e "${COLOUR_NONE}"
}
...
so_success () {
    ...
    >&2 echo -ne "${COLOUR_BACK_GREEN}@SUCCESS"
    [ -n "$1" ] && >&2 echo -ne " {$1}"
    >&2 echo -ne "${COLOUR_GREEN}" && shift && for arg in "$@"; do >&2 echo -ne " [$arg]"; done; >&2 echo -e "${COLOUR_NONE}"
}
...
so_error () {
    ...
    >&2 echo -ne "${COLOUR_BACK_BOLD_RED}@ERROR"
    [ -n "$1" ] && >&2 echo -ne " {$1}"
    >&2 echo -ne "${COLOUR_RED}" && shift && for arg in "$@"; do >&2 echo -ne " [$arg]"; done; >&2 echo -e "${COLOUR_NONE}"
}
...
so_show_source_line_variables () {
    set -x
}

so_hide_source_line_variables () {
    set +x
}